...
public class JedisUtilTest {
	...
	@Before
	public void setUp() {
		pool = new JedisPool(new JedisPoolConfig(), "192.168.1.173");
	...
	public void testBasicString() {
		...
		// 2、直接覆盖原来的数据
		jedis.set("name", "最中国");
		System.out.println(jedis.get("name"));// 执行结果：最中国
		...
		/**
		 * mset相当于 jedis.set("name","minxr"); jedis.set("jarorwar","最中国");
		 */
		jedis.mset("name", "minxr", "jarorwar", "最中国");
	...
	public void testMap() {
		...
		System.out.println(jedis.hlen("user")); // 返回key为user的键中存放的值的个数2，如果执行了jedis.hdel("user","pwd");返回值为1
	...
	public void test() throws InterruptedException {
		...
		jedis.setex("timekey", 20, "min");// 通过此方法，可以指定key的存活（有效时间） 时间为秒
